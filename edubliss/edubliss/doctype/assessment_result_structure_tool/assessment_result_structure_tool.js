...
frappe.ui.form.on('Assessment Result Structure Tool', {
	setup: function(frm) {
	...
	assessment_plan: function(frm) {
		...
		if(frm.doc.assessment_plan) {
			if (!frm.doc.student_group)
				return
			frappe.call({
				method: "edubliss.api.get_assessment_students",
				args: {
	...
	get_marks: function(frm, criteria_list) {
		...
		criteria_list.forEach(function(c) {
			max_total_score += c.max_score
		});
		var result_table = $(frappe.render_template('assessment_result_structure_tool', {
			frm: frm,
		...
		result_table.on('change', 'input', function(e) {
			...
			if (Object.keys(student_scores["assessment_details"]).length === criteria_list.length) {
				...
				frappe.call({
					method: "edubliss.api.mark_assessment_result",
					args: {
						"assessment_plan": frm.doc.assessment_plan,
					...
					callback: function(r) {
						...
						for (var criteria of Object.keys(assessment_result.custom_structure_detail)) {
							result_table.find(`[data-criteria=${criteria}][data-student=${assessment_result
								.student}].student-result-grade`).each(function(e1, input) {
									$(input).html(assessment_result.custom_structure_detail[criteria]);
							});
						}
